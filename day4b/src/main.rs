...
fn main() {
    ...
    for i in bingo_input {
        ...
        for j in 0..bingo_board_s.len() {
            ...
            if check_bingo(&bingo_board_s[j]) && (remaining_boards(&bingo_board_s) == 0) {
    ...
    // println!("Reamining Boards: {}", remaining_boards(&bingo_board_s));
    // println!("Bingo Input: {:?}", bingo_input);
    // println!("Bingo Boards: {:?}", bingo_board_s);
}
fn remaining_boards(input: &Vec<Vec<Vec<(usize, bool)>>>) -> usize {
    let mut output = 0;
    for x in input {
        if !check_bingo(x) {
            output = output + 1;
